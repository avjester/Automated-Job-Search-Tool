...
# from 12 to accommodate the Greenhouse and Workday API cross-checks: many
# candidate roles now cost a second (or, if the first attempt is blocked/
# inconclusive on Workday, effectively wasted) web_fetch call beyond the page
# itself, and Greenhouse + Workday together host the large majority of
# postings this pipeline finds — so a thorough run now needs noticeably more
# tool iterations than before, on top of watchlist checks (up to 32
# companies), an ATS-wide sweep across six job-board domains, and aggregator
# checks.
MAX_PAUSE_CONTINUATIONS = 18
...
GREENHOUSE-SPECIFIC ADDITIONAL CHECK, required, not optional: Greenhouse postings (any URL on job-boards.greenhouse.io or boards.greenhouse.io) have confirmed, reproducible cases where the rendered page displays a complete, convincing, fully-populated posting — full description, comp, an apparently-active apply form — for a role that a real browser (running Greenhouse's own client-side status check) reports as no longer active. A fetch of the page alone is NOT sufficient evidence of liveness for this platform; the rendered content can look entirely legitimate and still be wrong. For every Greenhouse posting, before including it, also fetch Greenhouse's public Job Board API endpoint for that same job: https://boards-api.greenhouse.io/v1/boards/{{board_token}}/jobs/{{job_id}} — the board_token is the path segment right after the domain in the posting URL (e.g. for job-boards.greenhouse.io/flex/jobs/4577672005, the board_token is "flex" and the job_id is 4577672005). This API requires no authentication.
- If that API call returns a successful response containing this job's id and details, treat this as confirming the job is genuinely still in the company's published set — proceed to include it as normal (subject to every other rule above).
- If that API call returns a 404 or otherwise indicates the job id is not found, treat this as HARD evidence the role is dead — same weight as an explicit "closed" message — and exclude it, even though the rendered page looked complete.
- If the API call fails for an unrelated reason (rate limiting, timeout, network error) rather than a clean not-found, this is inconclusive rather than dead — fall back to the page-fetch verification alone for that role, and if including it, note in the Fit field that the Greenhouse API cross-check could not be completed.

WORKDAY-SPECIFIC ADDITIONAL CHECK, required, not optional, but with MORE caution than the Greenhouse check above: Workday postings (any URL on {{tenant}}.{{wd-server}}.myworkdayjobs.com, e.g. trimble.wd1.myworkdayjobs.com) frequently render with little or no readable content on fetch at all — that case is already handled elsewhere (route to the manual check list, don't exclude). But when a Workday page DOES render enough content to look complete, the same "looks legitimate but might not be" risk applies as with Greenhouse. Workday has an internal JSON API used by its own careers-site UI that is reachable without authentication: given a posting URL of the form https://{{tenant}}.{{wd-server}}.myworkdayjobs.com/{{locale}}/{{site}}/job/{{job-path}}, the equivalent detail-check URL is https://{{tenant}}.{{wd-server}}.myworkdayjobs.com/wday/cxs/{{tenant}}/{{site}}/job/{{job-path}} (drop the locale segment, insert /wday/cxs/{{tenant}}/{{site}}/job/ before the job path). Try fetching that URL as an additional check before including a Workday role that otherwise looks complete.
- Unlike the Greenhouse check, treat a failure here MORE cautiously: Workday is known to run bot-detection that can block or rate-limit automated requests to this endpoint, which would look like a failure without meaning the role is actually dead. Only treat this check as hard evidence of death if the response is unambiguous (e.g. clearly indicates the job/requisition does not exist) — never treat a blocked request, a rate-limit response, a timeout, or any ambiguous/malformed response as evidence of death. If the check is blocked, inconclusive, or simply doesn't work, fall back to the page-fetch verification alone, and note in the Fit field that the Workday API cross-check could not be completed so the candidate knows to double-check this one by hand.
- If this check does succeed and clearly confirms the job doesn't exist, exclude the role, same as the Greenhouse case.

For a job on any other ATS platform (Ashby, Lever, SmartRecruiters, etc.), no equivalent public API is assumed — verify those using the page-fetch rules above only.
